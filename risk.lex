...
char** argv;
{
	...
	else
	{
		...
		else
		{
			...
			{
				char str[50];
				if(macro_list[i].params.num_params)
				{
					sprintf(str,"\"%s\"+\"(\"?+[^)]*+\")\"?",macro_list[i].name);
			...
			for(;i < macro_count;++i)
			{
				...
				{
					fprintf(in,"{MACRO%d}\t",i);
					fprintf(in,"\t\t{int i =0,j =0;char args[%d];",macro_list[i].params.num_params+1);
					
					  
					fprintf(in,"\nwhile(yytext[i]!='(')i++;i++;\nwhile(yytext[i]!=')')\n");
					fprintf(in,"{\nargs[j++] = yytext[i++];\nif(yytext[i] ==',')++i;}\nchar str[200];\nstrcpy(str,macro_params[%d].body);",i);
					fprintf(in,"int k =0;\nfor(;k<j;++k)\n{int i =0;\nfor(;str[i]!='\\0';++i)\nif(str[i] == macro_params[%d].params.param_list[k][0])\n{\nstr[i] = args[k];\n}\n}\nfprintf(out,\"%%s\",str);\n}\n\n",i);
					}
					}
			fprintf(in,"\"\\n\"\t\t{fprintf(out,\"\\n\");}");
